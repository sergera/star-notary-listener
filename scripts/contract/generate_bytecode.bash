...
# Description:
# Compiles EVM bytecode using solc docker image and write to build/contracts/CONTRACT_PACKAGE_NAME/bin
...
# Usage:
# generate_bytecode.bash SOLIDITY_VERSION CONTRACT_NAME CONTRACT_PACKAGE_NAME TRUFFLE_PROJECT_PATH
#
...
echo "generating contract bytecode..."
...
mkdir -p "build/contracts/$contract_package_name/bin" && 
docker pull "ethereum/solc:$solidity_version" && 
docker run --rm -v "$root_path:/root" -v "$truffle_root_path:/truffle" "ethereum/solc:$solidity_version" "openzeppelin-solidity=/truffle/node_modules/openzeppelin-solidity" --bin "/truffle/contracts/$contract_name.sol" --overwrite -o "/root/build/contracts/$contract_package_name/bin" &&
echo "contract bytecode generated successfully"
